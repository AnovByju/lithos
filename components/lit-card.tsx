...
export async function renderCardCanvas(item: LitItem, accent: string, thumbUrl: string | null): Promise<HTMLCanvasElement> {
  ...
  if (thumbUrl) {
    try { const img=await loadImg(thumbUrl); const s=Math.min(img.naturalWidth,img.naturalHeight); ctx.drawImage(img,(img.naturalWidth-s)/2,(img.naturalHeight-s)/2,s,s,cx,cy,AV,AV) }
  ...
  ctx.font="600 15px 'Helvetica Neue',Helvetica,sans-serif"; ctx.fillStyle="rgba(255,255,255,0.92)"; ctx.textAlign="left"
  ctx.fillText(item.author,cx+AV+14,cy+18); ctx.font="12px 'Helvetica Neue',Helvetica,sans-serif"; ctx.fillStyle="rgba(255,255,255,0.36)"
  ...
  ctx.font="11px 'Helvetica Neue',Helvetica,sans-serif"; ctx.fillStyle="rgba(255,255,255,0.22)"; ctx.textAlign="left"; ctx.fillText("↗  "+item.source,cx,cy+12)
...
export const LitCard = memo(function LitCard({
  ...
  return (
    // Full-screen container — card stretches to fill the whole viewport slot
    <div className="w-full flex items-stretch justify-center px-4 md:px-12"
      style={{ height: "100dvh", paddingTop: "calc(56px + env(safe-area-inset-top) + 12px)", paddingBottom: "calc(12px + env(safe-area-inset-bottom))" }}>
      <div
        className={cn(
          "relative w-full max-w-2xl rounded-3xl overflow-hidden flex flex-col",
      ...
      >
        {/* Accent top strip */}
        <div className="absolute top-0 left-0 right-0 h-[2px] opacity-70 z-10"
        ...
        {/* Inner content — flex-col, space distributed */}
        <div className="relative z-10 p-7 md:p-9 flex flex-col flex-1 gap-0">
          ...
          <div className="flex items-start justify-between gap-3 mb-5">
              ...
              <div className="w-11 h-11 rounded-full flex-shrink-0 overflow-hidden"
                ...
                {thumbUrl ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center"
                    style={{ background:`radial-gradient(circle at 30% 30%,${accent}44,${accent}22)`, fontFamily:"Times New Roman, serif", fontSize:"15px", color:"#fff" }}>
                    {initial}
                  </div>
                )}
              </div>
            ...
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full flex-shrink-0 text-[11px] uppercase"
              style={{ color:accent, border:`1px solid ${accent}35`, background:`${accent}12`, fontFamily:"Helvetica Neue, Helvetica, sans-serif", fontWeight:500, letterSpacing:"0.6px" }}>
          ...
          {/* ── Title ── */}
          {item.title && (
            <div className="text-[13px] italic mb-4"
          ...
          <div className={cn(
              "flex-1 leading-relaxed text-white/90 overflow-hidden",
              item.type === "quote" && "text-[22px] italic",
              item.type === "poem"  && "text-[16px] whitespace-pre-line",
              item.type !== "quote" && item.type !== "poem" && "text-[18px]",
            )}
            style={{ fontFamily:"Times New Roman, Times, serif", lineHeight:item.type==="poem"?1.8:1.78 }}>
          ...
          <div className="flex items-center justify-between gap-2 mt-6 pt-4 border-t"
            ...
            <div className="flex items-center gap-1.5 flex-shrink-0">
              {/* Swipe dots hint */}
              <div className="flex items-center gap-1 mr-1 opacity-25">
